...
export default function Project({ params }: { params: { project: string } }) {
	...
	console.log(rutas[2]);
	const rawdata = projs.filter((d) => {
		return d.slug == rutas[2];
	});
	const data: {
		name: string;
		slug?: string;
			...
			{data != undefined || null ? (
				<>
					<div className=' max-w-4xl flex flex-col gap-12 '>
						...
						<ProjHeader
							title={data.desc}
							role={data.tags != undefined ? data.tags : []}
...
En Datadis te conectamos con tus datos de consumo eléctrico, en cualquier distribuidora donde esté conectado tu suministro.'
						...
						<Section
							...
							<br />
							In addition to the structure, the way of addressing the user was not appropriate, we had to be precise with the terms used, but move away from the language
							excessively technical and legal. <br /> <br />
						...
						<div>
							...
							<div className='grid mt-2 gap-8 mb-8 grid-col-1 md:grid-cols-3 '>
								...
								<div className=' font-light text-2xl text-primary '>
									<div className='text-neutral  font-normal text-2xl mb-2'>#02</div> Data takes a long time to load. The user does not know what data exists or if there is an
									error.
								</div>
								<div className=' font-light text-2xl text-primary '>
